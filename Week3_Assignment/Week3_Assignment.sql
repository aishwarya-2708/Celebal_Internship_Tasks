...
-- Force disconnect all connections to SampleDB before dropping it
...
IF EXISTS (SELECT * FROM sys.server_principals WHERE name = 'SampleUserLogin')
BEGIN
    DROP LOGIN SampleUserLogin;
END
GO
...
-- Drop user if exists in SampleDB (to be safe, if script rerun in parts)
IF EXISTS (SELECT * FROM sys.database_principals WHERE name = 'SampleUser')
BEGIN
    DROP USER SampleUser;
END
GO

-- Create login and user again
...
-- Check roles assigned to SampleUser
SELECT 
